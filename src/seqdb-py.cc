...
PYBIND11_PLUGIN(seqdb_backend)
{
    ...
    py::class_<SeqdbSeq>(m, "SeqdbSeq")
            ...
            .def("amino_acids", &SeqdbSeq::amino_acids, py::arg("aligned"), py::arg("left_part_size") = 0, py::doc("if aligned and left_part_size > 0 - include signal peptide and other stuff to the left from the beginning of the aligned sequence."))
            .def("nucleotides", &SeqdbSeq::nucleotides, py::arg("aligned"), py::arg("left_part_size") = 0, py::doc("if aligned and left_part_size > 0 - include signal peptide and other stuff to the left from the beginning of the aligned sequence."))
    ...
    py::class_<SeqdbEntrySeq>(m, "SeqdbEntrySeq")
            ...
            .def("make_name", &SeqdbEntrySeq::make_name, py::arg("passage_separator") = " ")
    ...
    py::class_<Seqdb>(m, "Seqdb")
            ...
            .def("load", &Seqdb::load, py::arg("filename") = "", py::doc("reads seqdb from file containing json"))
            .def("json", &Seqdb::json, py::arg("indent") = 0)
            .def("save", &Seqdb::save, py::arg("filename") = "", py::arg("indent") = 0, py::doc("writes seqdb into file in json format"))
    ...
    py::class_<Text>(m, "Text")
            .def(py::init<const Location&, std::string, Color, double, const TextStyle&, double>(), py::arg("origin"), py::arg("text"), py::arg("color"), py::arg("size"), py::arg("style") = TextStyle(), py::arg("rotation") = 0)
    ...
    py::class_<Surface>(m, "Surface")
            ...
            .def("text", static_cast<void (Surface::*)(const Location&, std::string, Color, double, const TextStyle&, double)>(&Surface::text), py::arg("a"), py::arg("text"), py::arg("color"), py::arg("size"), py::arg("style") = TextStyle(), py::arg("rotation") = 0)
            .def("text", static_cast<void (Surface::*)(const Text&, const Viewport&)>(&Surface::text), py::arg("text"), py::arg("viewport"))
            .def("text_size", &Surface::text_size, py::arg("text"), py::arg("size"), py::arg("style") = TextStyle(), py::arg("x_bearing") = 0)
            ;
